...
public class AnimationConsistently : AnimationOpen
{
    private AnimationOpenType _animationOpenType = AnimationOpenType.Consistently;

    public AnimationOpenType AnimationOpenType => _animationOpenType;
    ...
    {
        // AnimationState nextState = _parentAnimationStateChanger.GetNextState();
        //
        // if (nextState == null || nextState.GetComponent<AnimationConsistently>() == null)
        // {
        ...
        _dotTransform.DOScale(Vector3.one, animationPointJson.time).OnComplete(() =>
        {
            _audioSource.Play();
            TryEnd();
        });
