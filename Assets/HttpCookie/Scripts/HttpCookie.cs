...
public static class HttpCookie
{
    ...
    private static extern string getURL(string name);
    [DllImport("__Internal")]
    private static extern string getAllUrlParams(string name);
    /// <summary>
    /// Получить все куки
    /// </summary>
    /// <returns></returns>
    ...
    [DllImport("__Internal")]
    private static extern void setHttpCookie(string name, string value, string expires, string path, string domain,
        string secure);
    ...
    {
        var dateJS = date.ToUniversalTime().ToString("ddd, dd'-'MMM'-'yyyy HH':'mm':'ss 'GMT'",
    ...
    public static string GetAllUrlParams(string name)
    {
        return getAllUrlParams(name);
    }
}
