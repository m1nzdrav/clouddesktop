...
namespace Doozy.Engine.UI
{
    ...
    public class UIView : UIComponentBase<UIView>
    {
        ...
        {
            var go = new GameObject(MenuUtils.UIView_GameObject_Name, typeof(RectTransform), typeof(UIView),
                typeof(Image));
        ...
        /// <summary> ActionEnter invoked whenever a UIViewBehavior is triggered </summary>
...
//            Debug.Log("CheckForLayoutController");
            if (!m_controlledByLayoutGroup)
            {
                ...
                else
                {
                    ...
                    if (!m_hasLayoutController)
                        m_layoutController = transform.parent.gameObject.AddComponent<LayoutController>();
                }
            }
            else
            {
...
//            if (HasChildUIViews && DisableGameObjectWhenHidden) DisableGameObjectWhenHidden = false;
            if (Settings.UseOrientationDetector && OrientationDetector != null)
                OrientationDetector.OnOrientationEvent
                    .AddListener(
                        OnOrientationChange); //if using OrientationDetector subscribe to the onOrientationChange UnityEvent every time this UIView is enabled
        }
        ...
        public override void OnDisable()
        {
            ...
            if (Settings.UseOrientationDetector && OrientationDetector != null)
                OrientationDetector.OnOrientationEvent
                    .RemoveListener(
                        OnOrientationChange); //if using OrientationDetector unsubscribe from the OnOrientationChange UnityEvent every time this UIView is disabled
        }
        ...
        public void Hide(bool instantAction = false)
        {
            ...
            {
                DDebug.Log(
                    "You are trying to HIDE the (" + ViewCategory + ")(" + ViewName +
                    ") UIView, but you did not enable any HIDE animations. Enable at least one HIDE animation in mlw to fix this issue.",
                    this);
        ...
        public void InstantHide()
        {
            ...
            GraphicRaycaster.enabled =
                !DisableGraphicRaycasterWhenHidden; //disable the graphic raycaster, if the option is enabled
            gameObject.SetActive(
                !DisableGameObjectWhenHidden); //disable the Source the UIView is attached to, if the option is enabled
        ...
        public void Show(bool instantAction = false)
        {
            ...
            {
                DDebug.Log(
                    "You are trying to SHOW the (" + ViewCategory + ")(" + ViewName +
                    ") UIView, but you did not enable any SHOW animations. Enable at least one SHOW animation in mlw to fix this issue.",
                    this);
...
//            RemoveNullChildUIButtons();
        ...
        private void OnOrientationChange(DetectedOrientation newDeviceOrientation)
        {
            if (newDeviceOrientation == DetectedOrientation.Landscape &&
                ...
                newDeviceOrientation == DetectedOrientation.Portrait &&
                TargetOrientation == TargetOrientation.Landscape)
            {
                HideView(ViewCategory, ViewName,
                    true); //hide all the views that have the same 'viewCategory' and 'viewName' instantly (in zero seconds)
                ShowView(ViewCategory, ViewName); //show all the views that have the same 'viewCategory' and 'viewName'
            }
            else if (TargetOrientation == TargetOrientation.Any
            ) //if this UIView targets both landscape and portrait orientations)
            {
                ShowView(ViewCategory, ViewName); //show all the views that have the same 'viewCategory' and 'viewName'
            }
        ...
        {
            if (m_childUIButtons == null || m_childUIButtons.Length <= 0)
            ...
            if (!foundNullEntry)
                return; //at least one null entries was found -> clean up the _childUIButtons array by removing all null entries
            m_childUIButtons =
                m_childUIButtons.Where(t => t != null)
                    .ToArray(); //set the new _childUIButtons as an array created from the temp list (that has no null entries)
        }
...
//            yield return null; //skip a frame
            ...
            //MOVE
            Vector3 moveFrom =
                UIAnimator.GetAnimationMoveFrom(RectTransform, ShowBehavior.Animation, CurrentStartPosition);
...
//            yield return null; //skip a frame
            ...
            Vector3 moveFrom =
                UIAnimator.GetAnimationMoveFrom(RectTransform, HideBehavior.Animation, CurrentStartPosition);
...
//                    yield return new WaitForSecondsRealtime(HideBehavior.Animation.TotalDuration + UIViewSettings.DISABLE_WHEN_HIDDEN_TIME_BUFFER); //wait for seconds realtime (ignore Unity's Time.Timescale)
                ...
                yield return
                    new WaitForSecondsRealtime(UIViewSettings
        ...
        {
            while (OrientationDetector.CurrentOrientation == DetectedOrientation.Unknown
            ) //if the current orientation is unknown
            {
                OrientationDetector
                    .CheckDeviceOrientation(); //check device orientation until we know if it's portrait or landscape
        ...
        private static void ExecuteHide(string viewCategory, string viewName, bool instantAction = false)
        {
            ...
            foreach (UIView view in Database)
            {
                ...
                if (DoozySettings.Instance.DebugUIView)
                    DDebug.Log("Hiding the (" + viewCategory + ")(" + viewName + ") UIView. This is " +
                               (instantAction ? "" : "NOT ") + "an instant action.");
                view.Hide(instantAction); //hide the UIView
            }

            if (foundNullEntry
            ) //well... it happened... at least one null UIView reference was found in the UIView.Database
        ...
        private static void ExecuteHideCategory(string viewCategory, bool instantAction = false)
        {
            ...
            foreach (UIView view in Database)
            {
                ...
                if (!view.gameObject.activeInHierarchy) continue;
                if (DoozySettings.Instance.DebugUIView)
                    DDebug.Log("Hiding the (" + viewCategory + ")(" + view.ViewName + ") UIView. This is " +
                               (instantAction ? "" : "NOT ") + "an instant action.");
                view.Hide(instantAction); //hide the UIView
            }
        ...
        private static void ExecuteShow(string viewCategory, string viewName, bool instantAction)
        {
            ...
            foreach (UIView view in Database)
            {
                ...
                if (!view.ViewName.Equals(viewName)) continue;
                view.gameObject
                    .SetActive(true); //set the Source active state to true (in case it has been disabled when hidden)
                if (!view.gameObject.activeInHierarchy) continue;
                if (!Settings.UseOrientationDetector) //check if the system is using the OrientationDetector
                {
                    if (DoozySettings.Instance.DebugUIView)
                        DDebug.Log("Showing the (" + viewCategory + ")(" + viewName + ") UIView. This is " +
                                   (instantAction ? "" : "NOT ") + "an instant action.");
                ...
                if (OrientationDetector.CurrentOrientation == DetectedOrientation.Unknown
                ) //check that we do not have an Unknown orientation
                    OrientationDetector
                        .CheckDeviceOrientation(); //do an orientation check to update the current orientation
                        ...
                        switch (view.TargetOrientation)
                        {
                            case TargetOrientation.Portrait:
                                if (DoozySettings.Instance.DebugUIView)
                                    DDebug.Log("Showing the (" + viewCategory + ")(" + viewName + ") UIView. This is " +
                                               (instantAction ? "" : "not ") + "an instant action." +
                                               (DoozySettings.Instance.UseOrientationDetector
                                                   ? " TargetOrientation: " + TargetOrientation.Portrait
                                                   : ""));
        ...
        private static void ExecuteShowCategory(string viewCategory, bool instantAction = false)
        {
            ...
            foreach (UIView view in Database)
            {
                ...
                {
                    if (DoozySettings.Instance.DebugUIView)
                        DDebug.Log("Showing the (" + viewCategory + ")(" + view.ViewName + ") UIView. This is " +
                                   (instantAction ? "" : "NOT ") + "an instant action.");
                    view.Show(instantAction); //show the UIView
                    continue; //continue to the next Database entry
                }
                ...
                switch (OrientationDetector.CurrentOrientation)
                {
                    case DetectedOrientation.Portrait:
                        switch (view.TargetOrientation)
                        {
                            case TargetOrientation.Portrait:
                                if (DoozySettings.Instance.DebugUIView)
                                    DDebug.Log("Showing the (" + viewCategory + ")(" + view.ViewName +
                                               ") UIView. This is " + (instantAction ? "" : "not ") +
                                               "an instant action." +
                                               (DoozySettings.Instance.UseOrientationDetector
                                                   ? " TargetOrientation: " + TargetOrientation.Portrait
                                                   : ""));
