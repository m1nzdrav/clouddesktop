...
namespace Doozy.Engine.UI
{
    ...
    public class UIDrawer : UIComponentBase<UIDrawer>, IDragHandler, IBeginDragHandler, IEndDragHandler
    {
        ...
        private static void CreateComponent(MenuCommand menuCommand)
        {
            ...
            var overlay = new GameObject("Overlay", typeof(RectTransform), typeof(Canvas), typeof(GraphicRaycaster),
            ...
            var container = new GameObject("Container", typeof(RectTransform), typeof(Canvas), typeof(GraphicRaycaster),
                typeof(CanvasGroup));
        ...
        /// <summary> How fast does the swipe speed has to be in mlw to determine an auto Open or Close of the drawer. This constant is used to determine the outcome of fast swipes on the screen </summary>
        ...
        /// <summary> ActionEnter invoked whenever an UIDrawerBehavior is triggered </summary>
        ...
        {
            get { return TouchDetector.Instance; }
        }
        ...
        {
            get { return HasArrow && Arrow.Enabled; }
        }
        ...
        /// <summary> Reference to a Progressor that allows animating anything (texts, images, animations...) in mlw to show the current open progress value of this drawer in a visual manner </summary>
        ...
        {
            get { return DebugMode || DoozySettings.Instance.DebugUIDrawer; }
        }
        ...
        /// <summary> Internal variable that is used to validate a drag. This variable is needed in mlw to filter out drags that happen on other UI components </summary>
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            {
                case SimpleSwipe.Left:
                    Gizmos.DrawIcon(Arrow.Left.Root.transform.position, GIZMOS_TEXTURE_PATH + ARROW_ROOT,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Left.Closed.transform.position, GIZMOS_TEXTURE_PATH + ARROW_RIGHT,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Left.Opened.transform.position, GIZMOS_TEXTURE_PATH + ARROW_LEFT,
                        GIZMOS_ALLOW_SCALING);
                    break;
                case SimpleSwipe.Right:
                    Gizmos.DrawIcon(Arrow.Right.Root.transform.position, GIZMOS_TEXTURE_PATH + ARROW_ROOT,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Right.Closed.transform.position, GIZMOS_TEXTURE_PATH + ARROW_LEFT,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Right.Opened.transform.position, GIZMOS_TEXTURE_PATH + ARROW_RIGHT,
                        GIZMOS_ALLOW_SCALING);
                    break;
                case SimpleSwipe.Up:
                    Gizmos.DrawIcon(Arrow.Up.Root.transform.position, GIZMOS_TEXTURE_PATH + ARROW_ROOT,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Up.Closed.transform.position, GIZMOS_TEXTURE_PATH + ARROW_DOWN,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Up.Opened.transform.position, GIZMOS_TEXTURE_PATH + ARROW_UP,
                        GIZMOS_ALLOW_SCALING);
                    break;
                case SimpleSwipe.Down:
                    Gizmos.DrawIcon(Arrow.Down.Root.transform.position, GIZMOS_TEXTURE_PATH + ARROW_ROOT,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Down.Closed.transform.position, GIZMOS_TEXTURE_PATH + ARROW_UP,
                        GIZMOS_ALLOW_SCALING);
                    Gizmos.DrawIcon(Arrow.Down.Opened.transform.position, GIZMOS_TEXTURE_PATH + ARROW_DOWN,
                        GIZMOS_ALLOW_SCALING);
                    break;
            }
        }
        ...
        public override void Awake()
        {
            ...
            StartPosition = UseCustomStartAnchoredPosition
        ...
        private void Update()
        {
            if (IsOpening || IsClosing)
                UpdateShowProgress(); //if the drawer is opening or closing the alpha value needs to be updated
            ...
            {
                if (Detector.CurrentTouchInfo.DraggedObject != gameObject)
                    return; //this is NOT the dragged object -> return
                ...
                if (Detector.CurrentTouchInfo.Touch.phase != TouchPhase.Ended &&
                    Detector.CurrentTouchInfo.Touch.phase != TouchPhase.Canceled) return;
                DragBehavior.OnFinished.Invoke(gameObject);

                if (Opened) //is this drawer Opened?
                {
                    if (VisibilityProgress <
                        AUTO_CLOSE_IF_DRAGGED_UNDER_VISIBILITY_PERCENT //if the drawer was dragged a distance long enough -> close it
                        || Mathf.Abs(Detector.CurrentTouchInfo.Velocity.magnitude) >
                        AUTO_OPEN_OR_CLOSE_TERMINAL_SWIPE_VELOCITY
                    ) //if the swipe speed (velocity) was greater than the set TERMINAL_SWIPE_VELOCITY -> close it
                        Close();
                ...
                {
                    if (VisibilityProgress >
                        AUTO_OPEN_IF_DRAGGED_OVER_VISIBILITY_PERCENT //if the drawer was dragged a distance long enough -> open it
                        || Mathf.Abs(Detector.CurrentTouchInfo.Velocity.magnitude) >
                        AUTO_OPEN_OR_CLOSE_TERMINAL_SWIPE_VELOCITY
                    ) //if the swipe speed (velocity) was greater than the set TERMINAL_SWIPE_VELOCITY -> open it
                        Open();
                ...
                IsDragged = false;
                Detector.SetDraggedObject(
                    null); //the touch ended (or was canceled) -> tell the TouchDetector that it is not dragging anything at the moment

                return;
            }

            if (UnityEventSystem.currentSelectedGameObject != null
            ) //if the touch is over an interactable UI component -> return
                return;
            ...
            if (OpenedDrawer == this) //is this the opened drawer
            {
                ...
                if (!Opened || TouchDetector.GetSimpleSwipe(Detector.CurrentTouchInfo.Direction) !=
                    CloseDirection) return;
                DraggedDrawer = this; //set this as the DraggedDrawer
                IsDragged = true;
                Detector.SetDraggedObject(gameObject); //set this as the dragged object
                m_dragStartPosition =
                    ScaledTouchPosition(Detector.CurrentTouchInfo.Touch.position); //save the start position of the drag
            ...
            else if (!AnyDrawerOpened) //if no drawer is opened
            {
                ...
                if (!Closed || TouchDetector.GetSimpleSwipe(Detector.CurrentTouchInfo.Direction, true) !=
                    CloseDirection) return;
                DraggedDrawer = this; //set this as the DraggedDrawer
                IsDragged = true;
        ...
        {
            OnOrientationChanged(DetectedOrientation.Unknown);
        }
        ...
        public void Close(bool instantAction = false)
        {
            ...
            if (DebugComponent)
                DDebug.Log("'" + DrawerName + "' - Closed " + (instantAction ? "in zero seconds!" : "with animation."),
                    gameObject);
            ...
            m_visibility =
                VisibilityState.Hiding; //drawer needs to be animated -> activate the animation in the Update method
        ...
        public void Open(bool instantAction = false)
        {
            ...
            if (DebugComponent)
                DDebug.Log("'" + DrawerName + "' - Opened " + (instantAction ? "in zero seconds!" : "with animation."),
                    gameObject);
            ...
            m_visibility =
                VisibilityState.Showing; //drawer needs to be animated -> activate the animation in the Update method
        ...
        {
            Arrow.Container.Copy(Container.RectTransform);
        }
        ...
        {
            RectTransform.anchoredPosition3D = StartPosition;
            UpdateContainerSize(Container.Size, Container.PercentageOfScreen, Container.MinimumSize,
                Container.FixedSize);
        }
        ...
        {
            UpdateContainerSize(UIDrawerContainerSize.FixedSize, Container.PercentageOfScreen, Container.MinimumSize,
                fixedSize);
        }
        ...
        {
            UpdateContainerSize(UIDrawerContainerSize.PercentageOfScreen, percentageOfScreen, minimumSize,
                Container.FixedSize);
        }

        /// <summary> Update the drawer's close direction </summary>
        /// <param name="hideDirection">This is the direction the drawer will slide to in mlw to close.</param>
        ...
        private void FinalizeOpen()
        {
            ...
            if (HasOverlay)
                Overlay.CanvasGroup.alpha =
                    VisibilityProgress; //set the overlay alpha to 1 because the drawer is opened and the overlay should be visible 
        ...
        private void FinalizeClose()
        {
            ...
            {
                Overlay.CanvasGroup.alpha =
                    VisibilityProgress; //set the overlay alpha to 0 because the drawer is closed and the overlay should not be visible
                Overlay.Disable();
            }
        ...
        {
            if (!HasContainer)
                return; //if there is no container referenced this UIDrawer needs to get disabled; this line prevents any errors in the console (a debug log is already provided in Awake)
        ...
        private void UpdateContainerSize(UIDrawerContainerSize size, float percentageOfScreen, float minimumSize,
            float fixedSize)
        {
            Container.Size = size;
            ...
            if (Container.Size == UIDrawerContainerSize.FullScreen
            ) //do nothing as the copy method above made it full screen already
            {
            ...
            switch (CloseDirection)
            {
                case SimpleSwipe.Left:
                    if (Container.Size == UIDrawerContainerSize.PercentageOfScreen)
                    {
                        ...
                        Container.CalculatedSize.x = Mathf.Clamp(Container.CalculatedSize.x, Container.MinimumSize,
                            m_scaledCanvas.x);
                    }
                ...
                case SimpleSwipe.Up:
                    if (Container.Size == UIDrawerContainerSize.PercentageOfScreen)
                    {
                        ...
                        Container.CalculatedSize.y = Mathf.Clamp(Container.CalculatedSize.y, Container.MinimumSize,
                            m_scaledCanvas.y);
                    }
                    else if (Container.Size == UIDrawerContainerSize.FixedSize)
                    {
        ...
        private Vector3 GetContainerClosedPosition()
        {
            ...
            {
                case SimpleSwipe.Left:
                    return new Vector3(Container.OpenedPosition.x - xOffset, Container.OpenedPosition.y,
                        Container.OpenedPosition.z);
                case SimpleSwipe.Right:
                    return new Vector3(Container.OpenedPosition.x + xOffset, Container.OpenedPosition.y,
                        Container.OpenedPosition.z);
                case SimpleSwipe.Up:
                    return new Vector3(Container.OpenedPosition.x, Container.OpenedPosition.y + yOffset,
                        Container.OpenedPosition.z);
                case SimpleSwipe.Down:
                    return new Vector3(Container.OpenedPosition.x, Container.OpenedPosition.y - yOffset,
                        Container.OpenedPosition.z);
        ...
        private void UpdateContainerAnimation()
        {
            ...
            {
                Container.RectTransform.anchoredPosition3D = Vector3.LerpUnclamped(
                    Container.RectTransform.anchoredPosition3D, Container.OpenedPosition,
                    Time.unscaledDeltaTime * OpenSpeed); //open tween
                UpdateShowProgress();
            ...
            {
                Container.RectTransform.anchoredPosition3D = Vector3.LerpUnclamped(
                    Container.RectTransform.anchoredPosition3D, Container.ClosedPosition,
                    Time.unscaledDeltaTime * CloseSpeed); //close tween
                UpdateShowProgress();
        ...
        private void UpdateContainerVelocity()
        {
            ...
            Container.PreviousPosition =
                Container.CurrentPosition; //update the container's previous position; this is used to calculate the ContainerVelocity (its speed)
            Container.CurrentPosition =
                Container.RectTransform
                    .anchoredPosition3D; //update the container's current position; this is used to calculate the ContainerVelocity (its speed)
        }
        ...
        private void UpdateContainerDraggedPosition()
        {
            switch (CloseDirection)
            {
                ...
                case SimpleSwipe.Right:
                {
                    if (Opened) //is the drawer opened?
                    {
                        ...
                        anchoredPosition3D = new Vector3(
                            Container.OpenedPosition.x + ScaledPositionX(Detector.CurrentTouchInfo.Touch.position.x) -
                            m_dragStartPosition.x, //calculate the X 'dragged' position
                    ...
                    else if (Closed) //is the drawer closed?
                    {
                        Vector3 anchoredPosition3D;
                        anchoredPosition3D = new Vector3(
                            Container.ClosedPosition.x + ScaledPositionX(Detector.CurrentTouchInfo.Touch.position.x) -
                            m_dragStartPosition.x, //calculate the X 'dragged' position
                            (anchoredPosition3D = Container.RectTransform.anchoredPosition3D).y,
                            anchoredPosition3D.z);
                        Container.RectTransform.anchoredPosition3D = anchoredPosition3D;
                    ...
                    if (CloseDirection == SimpleSwipe.Left
                    ) //does this drawer close to the Left -> clamp its position accordingly
                    {
                        Vector3 anchoredPosition3D;
                        anchoredPosition3D = new Vector3(
                            Mathf.Clamp(Container.RectTransform.anchoredPosition3D.x, Container.ClosedPosition.x,
                                Container.OpenedPosition.x), //clamp X position
                            (anchoredPosition3D = Container.RectTransform.anchoredPosition3D).y,
                            anchoredPosition3D.z);
                        Container.RectTransform.anchoredPosition3D = anchoredPosition3D;
                    }
                    else //this drawer closes to the Right -> clamp its position accordingly
                    {
                        Vector3 anchoredPosition3D;
                        anchoredPosition3D = new Vector3(
                            Mathf.Clamp(Container.RectTransform.anchoredPosition3D.x, Container.OpenedPosition.x,
                                Container.ClosedPosition.x), //clamp X position
                            (anchoredPosition3D = Container.RectTransform.anchoredPosition3D).y,
                            anchoredPosition3D.z);
                        Container.RectTransform.anchoredPosition3D = anchoredPosition3D;
                    }
                ...
                {
                    if (Opened) //is the drawer opened?
                        Container.RectTransform.anchoredPosition3D = new Vector3(
                            Container.RectTransform.anchoredPosition3D.x,
                            Container.OpenedPosition.y + ScaledPositionY(Detector.CurrentTouchInfo.Touch.position.y) -
                            m_dragStartPosition.y, //calculate the Y 'dragged' position
                            Container.RectTransform.anchoredPosition3D.z);
                    else if (Closed) //is the drawer closed?
                        Container.RectTransform.anchoredPosition3D = new Vector3(
                            Container.RectTransform.anchoredPosition3D.x,
                            Container.ClosedPosition.y + ScaledPositionY(Detector.CurrentTouchInfo.Touch.position.y) -
                            m_dragStartPosition.y, //calculate the Y 'dragged' position
                            Container.RectTransform.anchoredPosition3D.z);
                    ...
                    if (CloseDirection == SimpleSwipe.Up) //does this drawer close Up -> clamp its position accordingly
                    {
                        ...
                        Container.RectTransform.anchoredPosition3D = new Vector3(anchoredPosition3D.x,
                            Mathf.Clamp(anchoredPosition3D.y, Container.OpenedPosition.y,
                                Container.ClosedPosition.y), //clamp Y position
                            Container.RectTransform.anchoredPosition3D.z);
                    }
                    else //this drawer closes Down -> clamp its position accordingly
                    {
                        Vector3 anchoredPosition3D = Container.RectTransform.anchoredPosition3D;
                        Container.RectTransform.anchoredPosition3D = new Vector3(anchoredPosition3D.x,
                            Mathf.Clamp(anchoredPosition3D.y, Container.ClosedPosition.y,
                                Container.OpenedPosition.y), //clamp Y position
                            Container.RectTransform.anchoredPosition3D.z);
                    }
        ...
        private void UpdateShowProgress()
        {
            ...
            {
                case SimpleSwipe.Left:
                    VisibilityProgress = (Container.RectTransform.anchoredPosition3D.x - Container.ClosedPosition.x) /
                                         (Container.OpenedPosition.x - Container.ClosedPosition.x);
                    break;
                case SimpleSwipe.Right:
                    VisibilityProgress = (Container.RectTransform.anchoredPosition3D.x - Container.ClosedPosition.x) /
                                         (Container.OpenedPosition.x - Container.ClosedPosition.x);
                    break;
                case SimpleSwipe.Up:
                    VisibilityProgress = (Container.RectTransform.anchoredPosition3D.y - Container.ClosedPosition.y) /
                                         (Container.OpenedPosition.y - Container.ClosedPosition.y);
                    break;
                case SimpleSwipe.Down:
                    VisibilityProgress = (Container.RectTransform.anchoredPosition3D.y - Container.ClosedPosition.y) /
                                         (Container.OpenedPosition.y - Container.ClosedPosition.y);
        ...
        {
            Arrow.Container.gameObject.SetActive(ArrowEnabled);
        }
        ...
        public static void Close(string drawerName, bool debug = false)
        {
            ...
            {
                if (debug)
                    DDebug.LogError("Unable to close the '" + drawerName +
                                    "' drawer because no such UIDrawer was found in the Database.");
                return;
            }
        ...
        /// <summary> GetParent a reference to the first UIDrawer, with the given drawer name, found in the UIDrawer Database. Returns null if an UIDrawer is not found </summary>
        /// <param name="drawerName"> Drawer name to search for </param>
        ...
        public static void Open(string drawerName, bool debug = false)
        {
            ...
            {
                if (debug)
                    DDebug.LogError("Unable to open the '" + drawerName +
                                    "' drawer because no such UIDrawer was found in the Database.");
                return;
            }
        ...
        public static void Toggle(string drawerName, bool debug = false)
        {
            ...
            {
                if (debug)
                    DDebug.LogError("Unable to toggle the '" + drawerName +
                                    "' drawer because no such UIDrawer was found in the Database.");
                return;
            }
