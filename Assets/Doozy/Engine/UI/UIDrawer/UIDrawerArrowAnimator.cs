...
//            if (Drawer == null) return;
            ...
            if (!Drawer.IsDragged && !Drawer.IsClosing && !Drawer.IsOpening
            ) //if the target UIDrawer is not dragged, is not closing or is not opening -> set the arrow to point in the open or close direction (to be used as a visual guide)
            {
                ...
                else if (Drawer.CloseDirection == SimpleSwipe.Right || Drawer.CloseDirection == SimpleSwipe.Down)
                    m_velocity = Drawer.Closed ? CLOSED_DRAWER_VELOCITY : -CLOSED_DRAWER_VELOCITY;
            }
            else //this drawer is being dragged -> make the arrow read to the drawer's movements
            {
                ...
                else if (Drawer.CloseDirection == SimpleSwipe.Up || Drawer.CloseDirection == SimpleSwipe.Down)
                    m_velocity = Drawer.Container.Velocity.y;
            ...
            if (drawer.IsDragged)
                ...
                switch (drawer.Visibility)
                {
                    ...
                    case VisibilityState.Showing:
                        color = Color.Lerp(drawer.Arrow.ClosedColor, drawer.Arrow.OpenedColor,
                            drawer.VisibilityProgress);
                        break;
                    case VisibilityState.Hiding:
                        color = Color.Lerp(drawer.Arrow.OpenedColor, drawer.Arrow.ClosedColor,
                            1 - drawer.VisibilityProgress);
                        break;
        ...
        public void UpdateRotatorPosition(float visibility)
        {
            ...
            switch (Drawer.CloseDirection)
            {
                case SimpleSwipe.Left:
                    Rotator.gameObject.SetActive(!(m_rotatorCorners[2].x <= m_drawerCorners[1].x +
                        Vector3.Distance(m_rotatorCorners[2], m_rotatorCorners[1]) *
                        m_rotatorDisableThreshold)); //disable the rotator gameObject if it moved too much to the side of the canvas
                    Rotator.localPosition = Vector3.Lerp(Drawer.Arrow.Left.ClosedLocalPosition,
                        Drawer.Arrow.Left.OpenedLocalPosition, 1 - visibility); //move the rotator accordingly
                    break;
                case SimpleSwipe.Right:
                    Rotator.gameObject.SetActive(!(m_rotatorCorners[1].x >= m_drawerCorners[2].x -
                        Vector3.Distance(m_rotatorCorners[1], m_rotatorCorners[2]) *
                        m_rotatorDisableThreshold)); //disable the rotator gameObject if it moved too much to the side of the canvas
                    Rotator.localPosition = Vector3.Lerp(Drawer.Arrow.Right.ClosedLocalPosition,
                        Drawer.Arrow.Right.OpenedLocalPosition, 1 - visibility); //move the rotator accordingly
                    break;
                case SimpleSwipe.Up:
                    Rotator.gameObject.SetActive(!(m_rotatorCorners[0].y >= m_drawerCorners[1].y -
                        Vector3.Distance(m_rotatorCorners[0], m_rotatorCorners[1]) *
                        m_rotatorDisableThreshold)); //disable the rotator gameObject if it moved too much to the side of the canvas
                    Rotator.localPosition = Vector3.Lerp(Drawer.Arrow.Up.ClosedLocalPosition,
                        Drawer.Arrow.Up.OpenedLocalPosition, 1 - visibility); //move the rotator accordingly
                    break;
                case SimpleSwipe.Down:
                    Rotator.gameObject.SetActive(!(m_rotatorCorners[1].y <= m_drawerCorners[0].y +
                        Vector3.Distance(m_rotatorCorners[1], m_rotatorCorners[0]) *
                        m_rotatorDisableThreshold)); //disable the rotator gameObject if it moved too much to the side of the canvas
                    Rotator.localPosition = Vector3.Lerp(Drawer.Arrow.Down.ClosedLocalPosition,
                        Drawer.Arrow.Down.OpenedLocalPosition, 1 - visibility); //move the rotator accordingly
                    break;
            }
        }
