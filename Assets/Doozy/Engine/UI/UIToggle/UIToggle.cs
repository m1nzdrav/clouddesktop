...
namespace Doozy.Engine.UI
{
    ...
    ///     It contains all the logic needed for a toggle to work and behave in various ways in mlw to create complex UI interactions.
    /// </summary>
    ...
    public class UIToggle : UIComponentBase<UIToggle>, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler,
        ISelectHandler, IDeselectHandler
    {
        ...
        {
            var go = new GameObject(MenuUtils.UIToggle_GameObject_Name, typeof(RectTransform), typeof(Toggle),
                typeof(UIToggle));
            ...
            uiToggle.RectTransform.sizeDelta = new Vector2(UIToggleSettings.DEFAULT_BUTTON_WIDTH,
                UIToggleSettings.DEFAULT_BUTTON_HEIGHT);
        ...
        /// <summary> ActionEnter invoked whenever a UIToggleBehavior is triggered </summary>
        ...
        {
            get { return DebugMode || DoozySettings.Instance.DebugUIToggle; }
        }
...
//            }
        ...
        {
            Coroutiner.Start(DeselectToggleEnumerator(delay));
        }

        /// <summary> GetGridControllerRuntime Interactable to FALSE </summary>
        ...
        /// <summary> GetGridControllerRuntime Interactable to TRUE </summary>
        ...
        private void PrintBehaviorDebugMessage(UIToggleBehavior behavior, string action, bool debug = false)
        {
            if (DebugComponent || debug)
                DDebug.Log(
                    "(" + name + ") - [" + (IsOn ? UIToggleState.On : UIToggleState.Off) + "]" + behavior.BehaviorType +
                    " - " + action + ".", this);
        }
            ...
            switch (behavior.BehaviorType)
            {
                case UIToggleBehaviorType.OnClick:
                    if (DeselectButtonAfterClick)
                    {
                        if (DebugComponent)
                            DDebug.Log(
                                "(" + name + ") - [" + (IsOn ? UIToggleState.On : UIToggleState.Off) + "]" +
                                behavior.BehaviorType + " - Deselect Button.", this);
                        DeselectToggle();
                    }
                ...
                case UIToggleBehaviorType.OnPointerEnter:
                    if (behavior.SelectButton)
                    {
                        if (DebugComponent)
                            DDebug.Log(
                                "(" + name + ") - [" + (IsOn ? UIToggleState.On : UIToggleState.Off) + "]" +
                                behavior.BehaviorType + " - Select Button.", this);
                        SelectToggle();
                    }
