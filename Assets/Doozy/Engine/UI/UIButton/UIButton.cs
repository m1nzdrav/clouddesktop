...
namespace Doozy.Engine.UI
{
    ...
    ///     It contains all the logic needed for a button to work and behave in various ways in mlw to create complex UI interactions.
    /// </summary>
    ...
    public class UIButton : UIComponentBase<UIButton>, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler,
        IPointerUpHandler, IPointerClickHandler, ISelectHandler, IDeselectHandler
    {
        ...
        {
            CreateUIButton(GetParent(menuCommand.context as GameObject));
        }
        ...
        {
            var go = new GameObject(MenuUtils.UIButton_GameObject_Name, typeof(RectTransform), typeof(Image),
                typeof(Button), typeof(UIButton));
            ...
            uiButton.RectTransform.sizeDelta = new Vector2(UIButtonSettings.DEFAULT_BUTTON_WIDTH,
                UIButtonSettings.DEFAULT_BUTTON_HEIGHT);
        ...
        /// <summary> ActionEnter invoked whenever a UIButtonBehavior is triggered </summary>
        ...
        {
            get { return ButtonName.Equals(BackButtonName); }
        }
        ...
        {
            get { return UnityEventSystem != null && UnityEventSystem.currentSelectedGameObject == gameObject; }
        }
        ...
        {
            get { return m_updateStartValuesRequired; }
            set { m_updateStartValuesRequired = value; }
        }

        private bool DebugComponent
        {
            get { return DebugMode || DoozySettings.Instance.DebugUIButton; }
        }
        ...
        public UIButtonLoopAnimation SelectedLoopAnimation =
            new UIButtonLoopAnimation(ButtonLoopAnimationType.Selected);
            ...
            switch (InputData.InputMode)
            {
                ...
                case InputMode.VirtualButton:
                    if (UnityEngine.Input.GetButtonDown(InputData.VirtualButtonName) ||
                        InputData.EnableAlternateInputs &&
                        UnityEngine.Input.GetButtonDown(InputData.VirtualButtonNameAlt))
                        ExecuteClick();
        ...
        {
            Coroutiner.Start(DeselectButtonEnumerator(delay));
        }
        ...
        private IEnumerator DeselectButtonEnumerator(float delay)
        {
            if (Settings.IgnoreUnityTimescale) //check if the UI ignores Unity's Time.Timescale or not
                yield return
                    new WaitForSecondsRealtime(delay); //wait for seconds realtime (ignore Unity's Time.Timescale)
            else
        ...
        private IEnumerator ExecuteButtonBehaviorEnumerator(UIButtonBehavior behavior)
        {
            ...
            ) //on the first interaction update the start values so that the reset method works as intended 
            {
                UpdateStartValues();
            ...
            if (behavior.TriggerEventsAfterAnimation)
                yield return
                    new WaitForSecondsRealtime(behavior
                        .GetAnimationTotalDuration()); //wait for seconds realtime (ignore Unity's Time.Timescale)
            ...
            switch (behavior.BehaviorType)
            {
                ...
                case UIButtonBehaviorType.OnRightClick:
                    if (DeselectButtonAfterClick)
                    {
                        if (DebugComponent)
                            DDebug.Log(
                                "(" + ButtonName + ") UIButton - " + behavior.BehaviorType + " - Deselect Button.",
                                this);
                        DeselectButton();
                    }
                ...
                case UIButtonBehaviorType.OnPointerEnter:
                    ...
                    {
                        if (DebugComponent)
                            DDebug.Log("(" + ButtonName + ") UIButton - " + behavior.BehaviorType + " - Select Button.",
                                this);
                        SelectButton();
                    }
        ...
        private IEnumerator DisableButtonBehaviorEnumerator(UIButtonBehavior behavior)
        {
            ...
            if (Settings.IgnoreUnityTimescale) //check if the UI ignores Unity's Time.Timescale or not
                yield return
                    new WaitForSecondsRealtime(behavior
                        .DisableInterval); //wait for seconds realtime (ignore Unity's Time.Timescale)
            else
                yield return
                    new WaitForSeconds(behavior.DisableInterval); //wait for seconds (respect Unity's Time.Timescale)
            behavior.Ready = true;
        }
        ...
        private IEnumerator RunOnClickEnumerator(bool debug = false)
        {
            ...
            while (m_doubleClickTimeoutCounter < DoubleClickRegisterInterval)
            {
                ...
                if (Settings.IgnoreUnityTimescale) //check if the UI ignores Unity's Time.Timescale or not
                    m_doubleClickTimeoutCounter +=
                        Time.unscaledDeltaTime; //increment counter by change in time between frames (ignore Unity's Time.Timescale)
                else
                    m_doubleClickTimeoutCounter +=
                        Time.deltaTime; //increment counter by change in time between frames (respect Unity's Time.Timescale)
