    ...
    public abstract class UIComponentBase<T> : MonoBehaviour
    {
            ...
            if (selectedObject.transform.root.GetComponent<UICanvas>())
                return
                    selectedObject.transform.root
                        .gameObject; //selected object root has an UICanvas -> returns the root GameObject
            return
                UICanvas.GetMasterCanvas()
                    .gameObject; //selected object is NOT null, but has no UICanvas -> returns the MasterCanvas GameObject
        }
        ...
        public static EventSystem UnityEventSystem
        {
            get
            {
                ...
                if (s_unityEventSystem != null) return s_unityEventSystem;
                s_unityEventSystem = new GameObject("EventSystem", typeof(EventSystem), typeof(StandaloneInputModule))
