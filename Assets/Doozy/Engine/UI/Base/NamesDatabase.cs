...
//                CreateCategory(GENERAL, new List<string>(), false, false);
#if UNITY_EDITOR
                var listOfNames = AssetUtils.CreateAsset<ListOfNames>(GetPath(DatabaseType),
                    GetDatabaseFileName(DatabaseType, GENERAL));
#else
        ...
        public bool Contains(string categoryName)
        {
            ...
            if (Categories != null)
                return Categories.Where(listOfNames => listOfNames != null)
                    .Any(listOfNames => listOfNames.CategoryName.Equals(categoryName));
        ...
        /// <param name="saveAssets"> Write all unsaved asset changes to disk? </param>
        public bool CreateCategory(string categoryName, List<string> names, bool showDialog = false,
            bool saveAssets = false)
        {
            return CreateCategory(GetPath(DatabaseType), categoryName, names, showDialog, saveAssets);
        }
        ...
        /// <param name="showDialog"> Should a display dialog be shown before executing the action </param>
        /// <param name="saveAssets"> Write all unsaved asset changes to disk? </param>
        public bool CreateCategory(string relativePath, string categoryName, List<string> names,
            bool showDialog = false, bool saveAssets = false)
        {
            categoryName = categoryName.Trim();
...
#if UNITY_EDITOR
            var listOfNames =
                AssetUtils.CreateAsset<ListOfNames>(relativePath, GetDatabaseFileName(DatabaseType, categoryName));
#else
            ListOfNames listOfNames = ScriptableObject.CreateInstance<ListOfNames>();
#endif
        ...
        /// <param name="performUndo"> Record changes? </param>
        /// <param name="saveAssets"> Write all unsaved asset changes to disk? </param>
        public bool Rename(string oldCategoryName, string newCategoryName, bool performUndo = true,
            bool saveAssets = false)
        {
        ...
        public void SearchForUnregisteredDatabases(bool saveAssets)
        {
            ...
            {
                DoozyUtils.DisplayProgressBar(UILabels.SearchForDatabases, UILabels.Search,
                    0.1f + 0.7f * (i + 1) / array.Length);
        ...
        public void Sort(bool performUndo, bool saveAssets = false)
        {
            ...
            Categories =
                Categories.OrderBy(listOfNames => listOfNames.CategoryName).ToList(); //sort database by category name
        ...
        {
            CategoryNames.Clear();
            foreach (ListOfNames listOfNames in Categories.Where(listOfNames => listOfNames != null)
                .Where(listOfNames => !CategoryNames.Contains(listOfNames.CategoryName)))
            {
