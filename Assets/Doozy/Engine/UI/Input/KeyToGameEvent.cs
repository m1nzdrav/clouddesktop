...
namespace Doozy.Engine.UI.Input
{
    ...
    [AddComponentMenu(MenuUtils.KeyToGameEvent_AddComponentMenu_MenuName,
        MenuUtils.KeyToGameEvent_AddComponentMenu_Order)]
    ...
    public class KeyToGameEvent : MonoBehaviour
    {
        ...
        {
            get { return string.IsNullOrEmpty(GameEvent.Trim()) == false; }
        }

        private bool DebugComponent
        {
            get { return DebugMode || DoozySettings.Instance.DebugKeyToGameEvent; }
        }
            ...
            switch (InputData.InputMode)
            {
                case InputMode.KeyCode:
                    if (UnityEngine.Input.GetKeyDown(InputData.KeyCode))
                    {
                        ...
                        {
                            if (DebugComponent)
                                DDebug.Log("Sent '" + GameEvent + "' via KeyCode: " + InputData.KeyCode, gameObject);
                            execute = true;
                        }
                    }
                    else if (InputData.EnableAlternateInputs && UnityEngine.Input.GetKeyDown(InputData.KeyCodeAlt))
                    {
                        ...
                        {
                            if (DebugComponent)
                                DDebug.Log("Sent '" + GameEvent + "' via KeyCode: " + InputData.KeyCodeAlt, gameObject);
                            execute = true;
                        }
                    }
                ...
                case InputMode.VirtualButton:
                    if (UnityEngine.Input.GetButtonDown(InputData.VirtualButtonName))
                    {
                        if (HasGameEvent)
                        {
                            if (DebugComponent)
                                DDebug.Log(
                                    "Sent '" + GameEvent + "' via Virtual Button: " + InputData.VirtualButtonName,
                                    gameObject);
                            execute = true;
                        }
                    }
                    else if (InputData.EnableAlternateInputs &&
                             UnityEngine.Input.GetButtonDown(InputData.VirtualButtonNameAlt))
                    {
                        if (HasGameEvent)
                        {
                            if (DebugComponent)
                                DDebug.Log(
                                    "Sent '" + GameEvent + "' via Virtual Button: " + InputData.VirtualButtonNameAlt,
                                    gameObject);
                            execute = true;
                        }
                    }
        ...
        public void Execute()
        {
            GameEventMessage.SendEvent(GameEvent, gameObject);
        }
        ...
        {
            return DoozyUtils.AddToScene<KeyToGameEvent>(MenuUtils.KeyToGameEvent_GameObject_Name, false,
