...
namespace Doozy.Engine.UI.Input
{
    ...
    public class BackButton : MonoBehaviour
    {
        ...
        public bool CanExecuteBackButton
        {
            get
            {
                return Time.realtimeSinceStartup - m_lastBackButtonPressTime >= BACK_BUTTON_DETECTION_DISABLE_INTERVAL;
            }
        }

        private bool DebugComponent
        {
            get { return DebugMode || DoozySettings.Instance.DebugBackButton; }
        }
            ...
            switch (BackButtonInputData.InputMode)
            {
                case InputMode.KeyCode:
                    if (UnityEngine.Input.GetKeyDown(BackButtonInputData.KeyCode) ||
                        BackButtonInputData.EnableAlternateInputs &&
                        UnityEngine.Input.GetKeyDown(BackButtonInputData.KeyCodeAlt))
                    {
                        if (DebugComponent)
                            DDebug.Log("Back button detected via KeyCode: " + BackButtonInputData.KeyCode, Instance);
                ...
                case InputMode.VirtualButton:
                    if (UnityEngine.Input.GetButtonDown(BackButtonInputData.VirtualButtonName) ||
                        BackButtonInputData.EnableAlternateInputs &&
                        UnityEngine.Input.GetButtonDown(BackButtonInputData.VirtualButtonNameAlt))
                    {
                        if (DebugComponent)
                            DDebug.Log(
                                "Back button detected via Virtual Button: " + BackButtonInputData.VirtualButtonName,
                                Instance);
                        triggerBackButton = true;
                    }
        ...
        {
            return DoozyUtils.AddToScene<BackButton>(MenuUtils.BackButton_GameObject_Name, true,
