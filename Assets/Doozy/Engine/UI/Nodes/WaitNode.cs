...
namespace Doozy.Engine.UI.Nodes
{
    ...
    public class WaitNode : Node
    {
        ...
        {
            get { return base.HasErrors || ErrorNoGameEvent || ErrorNoSceneName || ErrorBadBuildIndex; }
        }
        ...
        public float TimerProgress
        {
            get
            {
                return Mathf.Clamp01(m_timerIsActive
                    ? (float) (Time.realtimeSinceStartup - m_timerStart) / m_timeDelay
                    : 0f);
            }
        }
            ...
            get
            {
                ...
                switch (WaitFor)
                {
                    ...
                    case WaitType.UIButton:
                        return AnyValue ? UILabels.AnyUIButton : ButtonCategory + " / " + ButtonName;
        ...
        {
            if (DebugMode)
                DDebug.Log("Active Scene Changed - Replaced Scene: " + current.name + " // Next Scene: " + next.name,
                    this);
        ...
        private void OnUIViewMessage(UIViewMessage message)
        {
            ...
            if (DebugMode)
                DDebug.Log(
                    "UIViewMessage received: " + message.Type + " " + message.View.ViewCategory + " / " +
                    message.View.ViewName + " // Listening for: " + ViewCategory + " / " + ViewName, this);
        ...
        private void OnUIButtonMessage(UIButtonMessage message)
        {
            ...
            if (DebugMode)
                DDebug.Log(
                    "UIButtonMessage received: " + message.Type + " " + message.ButtonName + " // Listening for: " +
                    ButtonName, this);
            if (message.Type != UIButtonTriggerAction) return;

            bool listeningForBackButton = ButtonName.Equals(UIButton.BackButtonName);
            if (listeningForBackButton && (message.ButtonName.Equals(UIButton.BackButtonName) ||
                                           message.Button != null && message.Button.IsBackButton))
            {
                ContinueToNextNode();
                return;
            }
            ...
            if (message.Button == null || !message.Button.ButtonCategory.Equals(ButtonCategory) ||
        ...
        private void OnUIDrawerMessage(UIDrawerMessage message)
        {
            ...
            if (DebugMode)
                DDebug.Log(
                    "UIDrawerMessage received: " + message.Type + " " + message.Drawer.DrawerName +
                    " // Listening for: " + DrawerName, this);
        ...
        public override void CheckForErrors()
        {
            ...
            switch (WaitFor)
            {
                ...
                case WaitType.ActiveSceneChange:
                    ...
                    ErrorBadBuildIndex = GetSceneBy == GetSceneBy.BuildIndex && (SceneBuildIndex < 0 ||
                        SceneBuildIndex + 1 > SceneManager.sceneCountInBuildSettings);
                    break;
            }
