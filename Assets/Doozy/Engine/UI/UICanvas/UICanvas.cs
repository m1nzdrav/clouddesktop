...
namespace Doozy.Engine.UI
{
    ...
    public class UICanvas : UIComponentBase<UICanvas>
    {
        ...
        private static void CreateComponent(MenuCommand menuCommand)
        {
            ...
            UICanvas[]
                searchResults =
                    FindObjectsOfType<UICanvas>(); //search the entire scene for any Source that may have an UICanvas component attached
            if (searchResults != null && searchResults.Length > 0
            ) //check if there isn't another UICanvas with the proposed canvasName
            {
        ...
        {
            get { return NamesDatabase.MASTER_CANVAS; }
        }
        ...
        {
            get { return CanvasName.Equals(MasterCanvasName) && GetMasterCanvas() == this; }
        }

        private bool DebugComponent
        {
            get { return DebugMode || DoozySettings.Instance.DebugUICanvas; }
        }
        ...
        public override void Awake()
        {
            ...
            {
                DDebug.LogError(
                    "The UICanvas, attached to the " + name +
                    " gameObject, does not have a Canvas component attached. Fix this by adding a Canvas component.",
                    this);
            ...
            {
                DDebug.LogError(
                    "The Canvas, attached to the " + name +
                    " gameObject, is to a root canvas. The UICanvas component must be attached to a top (root) canvas in the Hierarchy.",
                    this);
        ...
        public static UICanvas CreateUICanvas(string canvasName)
        {
            ...
            {
                DDebug.Log(
                    "You cannot create a new UICanvas without entering a 'canvasName'. The 'canvasName' you passed was an empty string. No UICanvas was created and returned null.");
                return null;
            }

            if (DatabaseContains(canvasName))
            {
                DDebug.Log("Cannot create a new UICanvas with the '" + canvasName +
                           "' canvasName because another UICanvas with the same name already exists in the UICanvas.Database. Returned the existing UICanvas instead.");
        ...
        {
            return Database.Any(t => t.CanvasName.Equals(canvasName));
        }
        ...
        public static UICanvas GetMasterCanvas(bool createMasterCanvasIfNotFound = true)
        {
            ...
            MasterCanvas =
                CreateUICanvas(
                    MasterCanvasName); //no UICanvas that has been registered to the UICanvas.Database has the name 'MasterCanvas' -> create a new UICanvas as the MasterCanvas and return it
        ...
        {
            return Database.FirstOrDefault(t => t.CanvasName.Equals(canvasName));
        }
        ...
        public static UICanvas GetUICanvas(string canvasName, bool createUICanvasIfNotFound,
            bool returnMasterCanvasIfUICanvasNotFound = true)
        {
            canvasName = canvasName.Trim();
            if (string.IsNullOrEmpty(canvasName))
            {
                DDebug.Log(
                    "You cannot search for an UICanvas without entering a 'canvasName'. The 'canvasName' you passed was an empty string. Returned null.");
                return null;
            }
