...
namespace Doozy.Engine.UI
{
    ...
    public class UIPopup : UIComponentBase<UIPopup>
    {
        ...
        private static void CreateComponent(MenuCommand menuCommand)
        {
            ...
            button.name = UIButtonSettings.Instance.RenamePrefix + button.ButtonName +
                          UIButtonSettings.Instance.RenameSuffix;
        ...
        {
            get { return UICanvas.MasterCanvasName; }
        }
        ...
        {
            get { return VisiblePopups.Count > 0 ? VisiblePopups[VisiblePopups.Count - 1] : null; }
        }

        // ReSharper disable once InconsistentNaming
        /// <summary> ActionEnter invoked whenever an UIPopupBehavior is triggered </summary>
        ...
        /// <summary> GetGridControllerRuntime by the UIPopupManager when this UIPopup (clone) is created (from the Prefab). Used for identification purposes </summary>
            ...
            set
            {
                ...
                switch (Visibility)
                {
                    case VisibilityState.Hiding:
                        ...
                        if (UpdateShowProgressorOnHide && ShowProgressor != null)
                            ShowProgressor.SetProgress(VisibilityProgress);
                        break;
                    case VisibilityState.Showing:
                        if (ShowProgressor != null) ShowProgressor.SetProgress(VisibilityProgress);
                        if (UpdateHideProgressorOnShow && HideProgressor != null)
                            HideProgressor.SetProgress(InverseVisibility);
                        break;
                    case VisibilityState.NotVisible:
                        if (HideProgressor != null) HideProgressor.SetProgress(InverseVisibility);
                        if (UpdateShowProgressorOnHide && ShowProgressor != null)
        ...
        {
            get { return DebugMode || DoozySettings.Instance.DebugUIPopup; }
        }
        ...
        ///     <para />
        ///     This is needed in mlw to restore the selection after this popup has been hidden.
        /// </summary>
        ...
        {
            return GetTargetCanvas(DisplayTarget, CanvasName);
        }
        ...
        public void Hide(bool instantAction = false)
        {
            ...
            {
                DDebug.Log(
                    "You are trying to HIDE the (" + name +
                    ") UIPopup, but you did not enable any HIDE animations. Enable at least one HIDE animation in mlw to fix this issue.",
                    this);
        ...
        public void Show(bool instantAction = false)
        {
            ...
            {
                DDebug.Log(
                    "You are trying to SHOW the (" + name +
                    ") UIPopup, but you did not enable any SHOW animations. Enable at least one SHOW animation in mlw to fix this issue.",
                    this);
        ...
        /// <summary> GetGridControllerRuntime the PopupName property. Used by the UIPopupManager to identify this UIPopup </summary>
        /// <param name="popupName"> The popup name designation for this UIPopup </param>
        public void SetPopupName(string popupName)
        {
            PopupName = popupName;
            if (DebugComponent) DDebug.Log("GetGridControllerRuntime PopupName: " + popupName, this);
        }

        /// <summary> GetGridControllerRuntime a new target canvas name and also re-parents this UIPopup under it (if the option is enabled) </summary>
        ...
        public void SetTargetCanvasName(string canvasName, bool reparentImmediately = true)
        {
            ...
            if (DebugComponent) DDebug.Log("GetGridControllerRuntime Target Canvas Name: " + canvasName, this);
        }
...
//            Hide(true);
        ...
        private void UpdateChildUIButtonsStartValues()
        {
            if (m_childUIButtons == null)
                return; //check that the m_childUIButtons array is not null and that we have a least one entry
        ...
        private IEnumerator ShowEnumerator(bool instantAction)
        {
            ...
            UIAnimator.StopAnimations(Container.RectTransform,
                ShowBehavior.Animation.AnimationType); //stop any SHOW animations
            Container.Enable(); //enable the gameobject, canvas and graphic raycaster
            Overlay.Enable();

            //MOVE
            Vector3 moveFrom = UIAnimator.GetAnimationMoveFrom(Container.RectTransform, ShowBehavior.Animation,
                Container.StartPosition);
            Vector3 moveTo = UIAnimator.GetAnimationMoveTo(Container.RectTransform, ShowBehavior.Animation,
                Container.StartPosition);
            if (!ShowBehavior.Animation.Move.Enabled || instantAction) Container.ResetPosition();
            UIAnimator.Move(Container.RectTransform, ShowBehavior.Animation, moveFrom, moveTo,
            ...
            UIAnimator.Rotate(Container.RectTransform, ShowBehavior.Animation, rotateFrom, rotateTo,
            ...
            UIAnimator.Scale(Container.RectTransform, ShowBehavior.Animation, scaleFrom, scaleTo,
            ...
            UIAnimator.Fade(Container.RectTransform, ShowBehavior.Animation, fadeFrom, fadeTo,
...
//            yield return null; //skip a frame
            ...
            UIAnimator.StopAnimations(Container.RectTransform,
                HideBehavior.Animation.AnimationType); //stop any HIDE animations

            //MOVE
            Vector3 moveFrom = UIAnimator.GetAnimationMoveFrom(Container.RectTransform, HideBehavior.Animation,
                Container.StartPosition);
            Vector3 moveTo = UIAnimator.GetAnimationMoveTo(Container.RectTransform, HideBehavior.Animation,
                Container.StartPosition);
            if (!HideBehavior.Animation.Move.Enabled || instantAction) Container.ResetPosition();
            UIAnimator.Move(Container.RectTransform, HideBehavior.Animation, moveFrom, moveTo,
            ...
            UIAnimator.Rotate(Container.RectTransform, HideBehavior.Animation, rotateFrom, rotateTo,
            ...
            UIAnimator.Scale(Container.RectTransform, HideBehavior.Animation, scaleFrom, scaleTo,
            ...
            UIAnimator.Fade(Container.RectTransform, HideBehavior.Animation, fadeFrom, fadeTo,
...
//                    yield return new WaitForSecondsRealtime(HideBehavior.Animation.TotalDuration + UIViewSettings.DISABLE_WHEN_HIDDEN_TIME_BUFFER); //wait for seconds realtime (ignore Unity's Time.Timescale)
                ...
                yield return
                    new WaitForSecondsRealtime(UIPopupSettings
                        .DISABLE_WHEN_HIDDEN_TIME_BUFFER); //wait for seconds realtime (ignore Unity's Time.Timescale)
            }
        ...
        {
            yield return null; //skip a frame
            m_previousSelectedButton =
                UnityEventSystem
                    .currentSelectedGameObject; //save the currently selected button (so that the selection can be restored when the popup is hidden)

            if (AutoSelectButtonAfterShow && SelectedButton != null
            ) //check that the auto select option is enabled and that a selectedButton has been referenced
        ...
        private IEnumerator ExecuteHideDeselectButtonEnumerator()
        {
            ...
            UnityEventSystem.SetSelectedGameObject(
                m_previousSelectedButton); //select the previously selected button (before the popup was shown)
        }
        ...
        {
            return popupDisplayOn == PopupDisplayOn.PopupCanvas
                ? GetPopupOverlayCanvas()
                : UICanvas.GetUICanvas(targetCanvasName);
        }
