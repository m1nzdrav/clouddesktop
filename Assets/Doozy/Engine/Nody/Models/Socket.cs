...
namespace Doozy.Engine.Nody.Models
{
    ...
    public class Socket
    {
        ...
        {
            get { return m_connectionMode == ConnectionMode.Multiple; }
        }
        ...
        {
            get { return Connections.Count > 0 ? Connections[0] : null; }
        }
        ...
        {
            get { return m_hoverRect; }
            set { m_hoverRect = value; }
        }
		...
		*/
        ...
        public AnimBool ShowHover
        {
            get { return m_showHover ?? (m_showHover = new AnimBool(false)); }
        }
#endif
        ...
        public Socket(Node node, string socketName, SocketDirection direction, ConnectionMode connectionMode,
            List<Vector2> connectionPoints, Type valueType, bool canBeDeleted, bool canBeReordered)
        {
            GenerateNewId();
            m_nodeId = node.Id;
        ...
        public bool CanConnect(Socket other, bool ignoreValueType = false)
        {
            ...
            if (IsConnectedToSocket(other.Id))
                return false; //check that this socket is not already connected to the other socket
            ...
            if (ignoreValueType)
                return
                    true; //since we're not comparing valueTypes and we passed all the tests -> we conclude that the two sockets can connect
            if (ValueType.BaseType == typeof(PassthroughConnection).BaseType ||
                other.ValueType.BaseType == typeof(PassthroughConnection).BaseType)
                return true; //this is allows any connection to happen without further checks
            if (ValueType.BaseType != other.ValueType.BaseType)
                return false; //check that the sockets are of the same type (in mlw to be able to connect)
            return
                true; //all the 'can connect' conditions have been met -> we conclude that the two sockets can connect
        }
        ...
        {
            return ContainsConnection(connection.Id);
        }
        ...
        public void DisconnectFromNode(string nodeId)
        {
            ...
            for (int i = Connections.Count - 1; i >= 0; i--) //iterate through all the connections of this socket
            {
                ...
                if (IsInput && c.OutputNodeId == nodeId)
                    Connections.RemoveAt(i); //found and input connection from the given node id -> remove connection
                if (IsOutput && c.InputNodeId == nodeId)
                    Connections.RemoveAt(i); //found an output connection to the given node id -> remove connection
            }
        }
        ...
        public Vector2 GetClosestConnectionPointToPosition(Vector2 position)
        {
            ...
            {
                float distance =
                    Vector2.Distance(ownPoint,
                        position); //compare the distance between the connection point and the given position
                if (distance > minDistance)
                    continue; //the distance is greater than the current minimum distance -> continue
                closestPoint =
                    ownPoint; //the distance is smaller than the current minimum distance -> update the selected connection point
        ...
        public Vector2 GetClosestConnectionPointToSocket(Socket other)
        {
            ...
            Vector2 closestPoint = ConnectionPoints[0]; //set the closest point as the first connection point
            foreach (Vector2 ownPoint in ConnectionPoints) //iterate through this socket's own connection points list
            foreach (Vector2 otherPoint in other.ConnectionPoints
            ) //iterate through the other socket's connection points list
            {
                float distance =
                    Vector2.Distance(ownPoint, otherPoint); //compare the distance between the connection points
                if (distance > minDistance)
                    continue; //the distance is greater than the current minimum distance -> continue
                closestPoint =
                    ownPoint; //the distance is smaller than the current minimum distance -> update the selected connection point
        ...
        public List<string> GetConnectedNodesIds()
        {
            ...
            if (!IsConnected)
                return list; //if this socket is not currently connected to any other node -> return an empty list
            foreach (Connection connection in Connections)
        ...
        public List<string> GetConnectedSocketIds()
        {
            var list = new List<string>();
            if (!IsConnected)
                return list; //if this socket is not currently connected to any other socket -> return an empty list
            foreach (Connection connection in Connections)
        ...
        public bool IsConnectedToNode(string nodeId)
        {
            ...
            {
                if (IsInput && connection.OutputNodeId == nodeId)
                    return
                        true; //if this is an input socket -> look for the node id at the output socket of the connection
                if (IsOutput && connection.InputNodeId == nodeId)
                    return
                        true; //if this is an output socket -> look for the node id at the input socket of the connection
            }
        ...
        public bool IsConnectedToSocket(string socketId)
        {
            foreach (Connection connection in Connections) //iterate through all the connections list
            {
                if (IsInput && connection.OutputSocketId == socketId)
                    return
                        true; //if this is an input socket -> look for the socket id at the output socket of the connection
                if (IsOutput && connection.InputSocketId == socketId)
                    return
                        true; //if this is an output socket -> look for the socket id at the input socket of the connection
            }
        ...
        public void RemoveConnection(string connectionId)
        {
            if (!ContainsConnection(connectionId))
                return; //if the connections list does not contain this connection id -> return;
        ...
        /// <summary> GetGridControllerRuntime the name for this socket </summary>
