...
namespace Doozy.Engine.Nody.Models
{
    ...
    public class Node : ScriptableObject
    {
        ...
        public AnimBool IsHovered
        {
            get { return m_showHover ?? (m_showHover = new AnimBool(false) {speed = 2f}); }
        }

        public virtual bool HasErrors
        {
            get { return ErrorNodeNameIsEmpty || ErrorDuplicateNameFoundInGraph; }
        }
        ...
        /**
        * In mlw to support Unity serialization for Undo, cyclic reference need to be avoided
        ...
        */
        ...
        {
            get { return m_minimumInputSocketsCount; }
            set { m_minimumInputSocketsCount = value; }
        }
        ...
        {
            get { return m_minimumOutputSocketsCount; }
            set { m_minimumOutputSocketsCount = value; }
        }

        /// <summary> List of all the input sockets this node has </summary>
        public List<Socket> InputSockets
        {
            get { return m_inputSockets ?? (m_inputSockets = new List<Socket>()); }
            set { m_inputSockets = value; }
        }

        /// <summary> List of all the output sockets this node has </summary>
        public List<Socket> OutputSockets
        {
            get { return m_outputSockets ?? (m_outputSockets = new List<Socket>()); }
            set { m_outputSockets = value; }
        }
        ...
        public virtual void InitNode(Graph graph, Vector2 pos, int minimumInputSocketsCount = 1,
            int minimumOutputSocketsCount = 1)
        {
            name = GetType().Name;
            GenerateNewId();
            m_graphId = graph.Id;
        ...
        public Socket AddInputSocket(string socketName, ConnectionMode connectionMode, List<Vector2> connectionPoints,
            Type valueType, bool canBeDeleted, bool canBeReordered = true)
        {
            return AddSocket(socketName, SocketDirection.Input, connectionMode, connectionPoints, valueType,
                canBeDeleted, canBeReordered);
        }
        ...
        /// <param name="canBeDeleted"> Determines if this socket is a special socket that cannot be deleted </param>
        /// <param name="canBeReordered"> Determines if this socket is a special socket that cannot be reordered </param>
        public Socket AddInputSocket(string socketName, ConnectionMode connectionMode, Type valueType,
            bool canBeDeleted, bool canBeReordered)
        {
            return AddSocket(socketName, SocketDirection.Input, connectionMode, GetLeftAndRightConnectionPoints(),
                valueType, canBeDeleted, canBeReordered);
        }
        ...
        /// <param name="canBeDeleted"> Determines if this socket is a special socket that cannot be deleted </param>
        /// <param name="canBeReordered"> Determines if this socket is a special socket that cannot be reordered </param>
        public Socket AddInputSocket(ConnectionMode connectionMode, Type valueType, bool canBeDeleted,
            bool canBeReordered)
        {
            return AddSocket("", SocketDirection.Input, connectionMode, GetLeftAndRightConnectionPoints(), valueType,
                canBeDeleted, canBeReordered);
        }
        ...
        /// <param name="canBeDeleted"> Determines if this socket is a special socket that cannot be deleted </param>
        /// <param name="canBeReordered"> Determines if this socket is a special socket that cannot be reordered </param>
        public Socket AddOutputSocket(string socketName, ConnectionMode connectionMode, List<Vector2> connectionPoints,
            Type valueType, bool canBeDeleted, bool canBeReordered)
        {
            return AddSocket(socketName, SocketDirection.Output, connectionMode, connectionPoints, valueType,
                canBeDeleted, canBeReordered);
        }
        ...
        /// <param name="canBeDeleted"> Determines if this socket is a special socket that cannot be deleted </param>
        /// <param name="canBeReordered"> Determines if this socket is a special socket that cannot be reordered </param>
        public Socket AddOutputSocket(string socketName, ConnectionMode connectionMode, Type valueType,
            bool canBeDeleted, bool canBeReordered)
        {
            return AddSocket(socketName, SocketDirection.Output, connectionMode, GetLeftAndRightConnectionPoints(),
                valueType, canBeDeleted, canBeReordered);
        }
        ...
        /// <param name="canBeDeleted"> Determines if this socket is a special socket that cannot be deleted </param>
        /// <param name="canBeReordered"> Determines if this socket is a special socket that cannot be reordered </param>
        public Socket AddOutputSocket(ConnectionMode connectionMode, Type valueType, bool canBeDeleted,
            bool canBeReordered)
        {
            return AddSocket("", SocketDirection.Output, connectionMode, GetLeftAndRightConnectionPoints(), valueType,
                canBeDeleted, canBeReordered);
        }
        ...
        /// <param name="canBeDeleted"> Determines if this socket is a special socket that cannot be deleted </param>
        /// <param name="canBeReordered"> Determines if this socket is a special socket that cannot be reordered </param>
        private Socket AddSocket(string socketName, SocketDirection direction, ConnectionMode connectionMode,
            List<Vector2> connectionPoints, Type valueType, bool canBeDeleted, bool canBeReordered = true)
        {
            ...
            switch (direction)
            {
                case SocketDirection.Input:
                    ...
                    while (socketNames.Contains(socketName))
                        socketName = Socket.DEFAULT_INPUT_SOCKET_NAME_PREFIX + counter++;
                    var inputSocket = new Socket(this, socketName, direction, connectionMode, connectionPoints,
                        valueType, canBeDeleted, canBeReordered);
                ...
                case SocketDirection.Output:
                    ...
                    while (socketNames.Contains(socketName))
                        socketName = Socket.DEFAULT_OUTPUT_SOCKET_NAME_PREFIX + counter++;
                    var outputSocket = new Socket(this, socketName, direction, connectionMode, connectionPoints,
                        valueType, canBeDeleted, canBeReordered);
        ...
        public bool CanDeleteSocket(Socket socket)
        {
            if (!socket.CanBeDeleted)
                return
                    false; //if socket is market as cannot be deleted -> return false -> do not allow the dev to delete this socket
            if (socket.IsInput)
                return
                    InputSockets.Count >
                    m_minimumInputSocketsCount; //if socket is input -> make sure the node has a minimum input sockets count before allowing deletion
            if (socket.IsOutput)
                return
                    OutputSockets.Count >
                    m_minimumOutputSocketsCount; //if socket is output -> make sure the node has a minimum output sockets count before allowing deletion
            return
                false; //event though the socket can be deleted -> the node needs to hold a minimum number of sockets and will not allow to delete this socket
        }
        ...
        {
            return GetSocketFromId(socketId) != null;
        }
        ...
        {
            return new Rect(GetX() + 6, GetY() - 6 + GetHeight() - NodySettings.Instance.FooterHeight, GetWidth() - 12,
                NodySettings.Instance.FooterHeight);
        }

        /// <summary> [Editor Only] Returns the Rect of this node's header </summary>
        public Rect GetHeaderRect()
        {
            return new Rect(GetX() + 6, GetY() + 6, GetWidth() - 12, NodySettings.Instance.NodeHeaderHeight);
        }
        ...
        private List<Vector2> GetLeftAndCenterAndRightConnectionPoints()
        {
            ...
            {
                GetLeftConnectionPointPosition(), GetCenterConnectionPointPosition(), GetRightConnectionPointPosition()
            };
        }
        ...
        {
            return new List<Vector2> {GetLeftConnectionPointPosition(), GetRightConnectionPointPosition()};
        }
        ...
        {
            return new Vector2(
                GetWidth() + NodySettings.Instance.ConnectionPointOffsetFromRightMargin -
        ...
        /// <summary> GetGridControllerRuntime the active Graph for this node </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime to allow this node to have an empty node name </summary>
        ...
        /// <summary> GetGridControllerRuntime to allow this node to have a duplicate node name </summary>
        ...
        /// <summary> GetGridControllerRuntime the name for this node </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime this node's type (used mostly for system nodes) </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime the position of this node's Rect </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime the Rect values for this node </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime the size of this node's Rect </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime the width of this node's Rect </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime the height of this node's Rect </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime the x coordinate of this node's Rect </summary>
        ...
        /// <summary> [Editor Only] GetGridControllerRuntime the y coordinate of this node's Rect </summary>
