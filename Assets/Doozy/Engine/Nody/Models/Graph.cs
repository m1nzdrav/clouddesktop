...
namespace Doozy.Engine.Nody.Models
{
    ...
    public class Graph : ScriptableObject
    {
        ...
        public const int
            FILE_VERSION = 1; //Important: FILE_VERSION must be increased by one when any structure change(s) happen
        ...
        {
            get { return m_globalNodes ?? (m_globalNodes = new List<Node>()); }
        }

        /// <summary> Keeps track of all the Nodes this graph contains </summary>
        public List<Node> Nodes
        {
            get { return m_nodes ?? (m_nodes = new List<Node>()); }
            set { m_nodes = value; }
        }
        ...
        {
            get { return GlobalNodes.Count > 0; }
        }
        ...
        {
            get { return m_isSubGraph; }
            set { m_isSubGraph = value; }
        }
...
//            ActivateGlobalNodes();
        ...
        {
            return GetNodeByName(nodeName) != null;
        }
        ...
        {
            return GetSocket(socketId) != null;
        }
        ...
        {
            return m_isSubGraph ? GetEnterNode() : GetStartNode();
        }
        ...
        public void SetActiveNode(Node nextActiveNode, Connection connection = null)
        {
            ...
            if (InfiniteLoopDetected(nextActiveNode))
                throw new OverflowException("Nody detected an Infinite Loop." +
                                            "\n" +
                                            "This loop was detected in the '" + name + "' " +
                                            (m_isSubGraph ? "SubGraph" : "Graph") + ", at the '" + nextActiveNode.Name +
                                            "' Node. " +
                                            (PreviousActiveNode == null
                                                ? ""
                                                : "The connection that triggered this exception was between '" +
                                                  PreviousActiveNode.Name + "' --and--> '" + nextActiveNode.Name +
                                                  "' Nodes. ") +
                                            "This issue happened because the sockets are connected in such a manner that a closed infinite loop was created. " +
                                            "Please redesign the current UI flow to and from the '" +
                                            nextActiveNode.Name +
                                            "' Node, because Nody cannot automatically do that for you. " +
                                            "Look at how the '" + nextActiveNode.Name +
                                            "' Node is connected to other nodes and try to follow the UI flow in mlw to understand where the infinite loop happens. " +
        ...
        {
            SetActiveNode(GetNodeById(connection.InputNodeId), connection);
        }
        ...
        {
            SetActiveNode(GetNodeById(nodeId), connection);
        }
        ...
        {
            SetActiveNode(GetNodeByName(nodeName), connection);
        }
