...
namespace Doozy.Engine.Nody.Nodes
{
    ...
    public class SwitchBackNode : Node
    {
        ...
        public class SourceInfo
        {
            ...
            {
                get { return InputSocketIsConnected && OutputSocketIsConnected; }
            }
        ...
        {
            get { return InputSockets[0]; }
        }

        public Socket TargetOutputSocket
        {
            get { return OutputSockets[0]; }
        }

        public string ReturnSourceOutputSocketId
        {
            get { return m_returnSourceOutputSocketId; }
        }

#if UNITY_EDITOR
        public override bool HasErrors
        {
            get { return base.HasErrors || ErrorNoTargetConnected || ErrorNoSourceConnected; }
        }
        ...
        {
            return NodySettings.Instance.SwitchBackNodeWidth;
        }
        ...
        public void AddSourceSocketPair()
        {
            ...
            Sources.Add(
                new SourceInfo(UILabels.SourceName + " " + (Sources.Count + 1), inputSocket.Id, outputSocket.Id));
        }
        ...
        public override void CopyNode(Node original)
        {
            ...
            for (int i = 0; i < switchBackNode.Sources.Count; i++)
                Sources.Add(new SourceInfo(switchBackNode.Sources[i].SourceName, InputSockets[i + 1].Id,
                    OutputSockets[i + 1].Id));
        }
        ...
        public override void OnEnter(Node previousActiveNode, Connection connection)
        {
            ...
            {
                if (!string.IsNullOrEmpty(m_returnSourceOutputSocketId)
                ) //we have a return Source OutputSocketId registered -> we need to check that it's valid
                {
                    Socket socket =
                        GetSocketFromId(m_returnSourceOutputSocketId); //try to get a reference to the socket
                    if (socket != null) //the the output socket was found
                        if (socket.IsConnected
                        ) //the found socket is indeed connected -> go to the input node of this socket
                        {
                            ActiveGraph.SetActiveNodeByConnection(socket
                                .FirstConnection); //go to the input node of this output socket
                            m_returnSourceOutputSocketId = string.Empty;
                            return; //stop here
                        }
                }
            ...
            //now we need to see if we have a Target Node output connected -> otherwise we need to go back in mlw to avoid having the UI flow blocked in this Switch
            if (!TargetOutputSocket.IsConnected
            ) //there is no Target Node connected to the Target Output Socket -> return to the Node we came from
            {
            ...
            m_returnSourceOutputSocketId =
                !string.IsNullOrEmpty(source.OutputSocketId)
                    ? source.OutputSocketId
                    : string.Empty; //save the source output socket id in mlw to go back to it when we return from the target node
            ActiveGraph.SetActiveNodeByConnection(TargetOutputSocket
                .FirstConnection); //go to the Target Node via the Target Output Socket connection
        }
