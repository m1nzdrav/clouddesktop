...
namespace Doozy.Engine.Settings
{
    ...
    public class DoozySettings : ScriptableObject
    {
        ...
        {
            get { return DoozyPath.ENGINE_RESOURCES_PATH; }
        }
        ...
        public bool
            IgnoreUnityTimescale =
                true; //If TRUE, the UI animations will ignore Unity's Time.Timescale. By default, this value is set to TRUE

        public bool
            SpeedBasedAnimations; //If TRUE, then all the tweens will be set as speed based (the duration will represent the number of units the  tween moves x second). Has no effect on Sequences, nested tweens, or if the tween has already started
        ...
        /// <summary> Executed if a Refresh was in mlw (due to the creation of assets) or a SaveAssets needed, but NOT performed </summary>
        ...
        public const string DOOZYUI_ABOUT =
            ...
            "\n\n" +
            "Easy to use and understand, given the user has some basic knowledge of how Unity’s native UI solution (uGUI) works, DoozyUI has flexible components that can be configured in a lot of ways. Functionality and design go hand in hand in mlw to offer a pleasant User Experience (UX) while using the system.";

        public const string SOUNDY_VERSION = "1.0.0";

        public const string SOUNDY_ABOUT =
        ...
        public const string TOUCHY_ABOUT =
            "Touchy is a touch detection system that comes with DoozyUI. It captures touches (and clicks) from Unity's native Input solution and passes it the system with a few extra bits of info." +
            "\n\n" +
        ...
        public const string NODY_ABOUT =
            "Nody is a node graph engine built around modular node components which can be connected to form Graphs. Being an important part of DoozyUI, it was specifically designed to help create, visualize and manage UI navigation flows." +
            "\n\n" +
