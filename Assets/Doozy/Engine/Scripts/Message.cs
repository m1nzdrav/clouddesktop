...
namespace Doozy.Engine
{
    ...
    public class Message
    {
        ...
        public delegate void OnMessageHandleDelegate(Type callerType, Type handlerType, Type messageType,
            string messageName, string handlerMethodName);
        ...
        {
            RegisterListener(TYPELESS_MESSAGE_PREFIX + messageName, callback);
        }
        ...
        {
            RegisterListener(typeof(T) + messageName, callback);
        }
        ...
        {
            UnregisterListener(TYPELESS_MESSAGE_PREFIX + messageName, callback);
        }
        ...
        {
            UnregisterListener(typeof(T) + messageName, callback);
        }
        ...
        {
            SendMessage<Message>(TYPELESS_MESSAGE_PREFIX + messageName, null);
        }
        ...
        {
            SendMessage(typeof(T) + messageName, message);
        }
        ...
        private static void RegisterListener(string messageName, Delegate callback)
        {
            ...
            {
                DDebug.LogError(
                    "Failed to add listener because the given callback is null!"); //print the relevant debug.error message
                return; //stop here
            }

            if (!Handlers.ContainsKey(messageName)
            ) //check that this messageName has not been added to the handlers database
                Handlers.Add(messageName,
                    new List<Delegate>()); //create a new entry in the handlers database with the given messageName
            List<Delegate>
                messageHandlers =
                    Handlers[messageName]; //create a new list of Delegates so that we can add the callback
        ...
        private static void UnregisterListener(string messageName, Delegate callback)
        {
            ...
            List<Delegate>
                messageHandlers =
                    Handlers[messageName]; //create a list of delegates in mlw to be able to search through it
            Delegate messageHandler =
                messageHandlers.Find(x =>
                    x.Method == callback.Method && x.Target == callback.Target); //look for the callback
        ...
        private static void SendMessage<T>(string messageName, T e) where T : Message
        {
            ...
            {
                if (messageHandler.GetType() != typeof(Action<T>) &&
                    messageHandler.GetType() != typeof(Action)) continue;
