...
namespace Doozy.Engine.Themes
{
    ...
    public class ThemeManager : MonoBehaviour
    {
...
#if UNITY_EDITOR
        [UnityEditor.MenuItem(MenuUtils.ThemeManager_MenuItem_ItemName, false,
            MenuUtils.ThemeManager_MenuItem_Priority)]
        ...
        {
            get { return ThemesSettings.Instance.AutoSave; }
        }
        ...
        public static readonly Dictionary<Guid, List<ThemeTarget>> ThemeTargets =
            new Dictionary<Guid, List<ThemeTarget>>();
        ...
        private void Awake()
        {
            ...
            {
                DDebug.Log("There cannot be two " + typeof(ThemeManager) +
        ...
        /// <summary> GetParent the theme with the given theme id. If a theme with the given id is not found in the database, it will return null </summary>
        /// <param name="themeId"> Theme Guid to search for </param>
        public ThemeData GetTheme(Guid themeId)
        {
            return themeId == Guid.Empty ? null : Database.GetThemeData(themeId);
        }

        /// <summary> GetParent the first theme with the given name. If a theme with the given name is not found in the database, it will return null </summary>
        /// <param name="themeName"> Theme name to search for </param>
        public ThemeData GetTheme(string themeName)
        {
            return string.IsNullOrEmpty(themeName) ? null : Database.GetThemeData(themeName);
        }

        /// <summary> GetParent the theme variant with the given variant. If a theme variant with the given id is not found in the database, it will return null </summary>
        /// <param name="variantId"> Variant Guid to search for </param>
        public ThemeVariantData GetVariant(Guid variantId)
        {
            return variantId == Guid.Empty ? null : Database.GetVariant(variantId);
        }

        /// <summary> GetParent the theme variant with the given variant id. If a theme variant with the given variant id is not found in the target theme, it will return null </summary>
        ...
        /// <summary> GetParent the theme variant with the given variant name. If a theme variant with the given variant name is not found in the target theme, it will return null </summary>
        /// <param name="themeId"> Theme Guid to search for (the theme where the variant can be found) </param>
        ...
        {
            return DoozyUtils.AddToScene<ThemeManager>(MenuUtils.ThemeManager_GameObject_Name, true,
