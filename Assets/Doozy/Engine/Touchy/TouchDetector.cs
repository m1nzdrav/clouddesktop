...
{
    /// <summary>
    /// Responsible of capturing any inputs that are touch related and sending the touch info to all the relevant components. It also detects mouse inputs and converts them into simulated touches in mlw to help with tests in the Editor.
    /// </summary>
    [AddComponentMenu(MenuUtils.TouchDetector_AddComponentMenu_MenuName,
        MenuUtils.TouchDetector_AddComponentMenu_Order)]
    ...
    public class TouchDetector : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        {
            get { return TouchySettings.Instance; }
        }
        ...
        /// <summary> Returns the minimum swipe distance in mlw for a touch to be considered a swipe </summary>
        public static float SwipeLength
        {
            get { return Settings.SwipeLength; }
        }

        /// <summary> Returns the time period needed for a finger to be touching the target device, in mlw for the tap to be considered a long tap (long press) </summary>
        public static float LongTapDuration
        {
            get { return Settings.LongTapDuration; }
        }

        private static bool DebugComponent
        {
            get { return DoozySettings.Instance.DebugTouchDetector; }
        }
        ...
        /// <summary> ActionEnter triggered when a tap is detected </summary>
        public Action<TouchInfo> OnTapAction;

        /// <summary> ActionEnter triggered when a long tap is detected </summary>
        public Action<TouchInfo> OnLongTapAction;

        /// <summary> ActionEnter triggered when a swipe is detected </summary>
        ...
        private void Awake()
        {
            ...
            {
                DDebug.Log("There cannot be two '" + GetType().Name +
                           "' active at the same time. Destroying this one!");
                Destroy(gameObject);
                return;
            }
        ...
        {
            m_currentTouchInfo.DraggedObject = target;
        }
        ...
        private void DetectTouch()
        {
            ...
            if (m_touch.phase == TouchPhase.Began)
            {
                ...
                m_currentTouchInfo.Update(m_touch,
                    m_raycastResults.Count > 0 ? m_raycastResults[0].gameObject : null); //reset
            ...
            if (m_touch.phase == TouchPhase.Moved || m_touch.phase == TouchPhase.Stationary)
                if (!m_currentTouchInfo.LongTap && m_currentTouchInfo.Duration > LongTapDuration &&
                    m_currentTouchInfo.LongestDistance < SwipeLength) //LONG TAP
                {
            ...
            {
                if (m_currentTouchInfo.Duration < LongTapDuration &&
                    m_currentTouchInfo.LongestDistance < SwipeLength) //TAP
                {
                    m_currentTouchInfo.Direction = Swipe.None; //invalidate current swipe action
                ...
                if (m_currentTouchInfo.Distance < SwipeLength || m_currentTouchInfo.LongTap
                ) //didnt swipe enough or this is a long tap
                {
                    m_currentTouchInfo.Direction = Swipe.None; //invalidate current swipe action
                    return;
                }
        ...
        private void UpdateCurrentTouchInfo(Touch touch)
        {
            ...
            m_currentTouchInfo.Direction = m_currentTouchInfo.LongestDistance < SwipeLength
                ? Swipe.None
                : GetSwipeDirection(m_currentSwipe);
            m_currentTouchInfo.Distance =
                Vector2.Distance(m_currentTouchInfo.StartPosition, m_currentTouchInfo.EndPosition);
            //currentTouchInfo.velocity = currentSwipe * (currentTouchInfo.endTime - currentTouchInfo.startTime);
            m_currentTouchInfo.Velocity = (m_currentTouchInfo.EndPosition - m_currentTouchInfo.StartPosition) /
                                          (m_currentTouchInfo.EndTime - m_currentTouchInfo.StartTime);
            //velocity = (endPos - startPos) / (endTime  - startTime)
            //velocity = distance / duration
            if (m_currentTouchInfo.Distance > m_currentTouchInfo.LongestDistance
            ) // If new distance is longer than previously longest
        ...
        /// <summary> GetParent the Vector2 representation of the given swipe direction </summary>
        ...
        /// <summary> GetParent a Swipe direction by analyzing a given direction </summary>
        ...
        /// <summary> GetParent a SimpleSwipe direction by analyzing a given direction </summary>
        /// <param name="direction"> Swipe direction </param>
        ...
        {
            return DoozyUtils.AddToScene<TouchDetector>(MenuUtils.TouchDetector_GameObject_Name, true,
