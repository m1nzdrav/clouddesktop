...
namespace Doozy.Engine.Soundy
{
    ...
    public class SoundyDatabase : ScriptableObject
    {
        ...
        {
            return Contains(databaseName) && GetSoundDatabase(databaseName).Contains(soundName);
        }
        ...
        {
            return CreateSoundDatabase(DoozyPath.GetDataPath(DoozyPath.ComponentName.Soundy), databaseName, showDialog,
                saveAssets);
        }
        ...
        /// <param name="showDialog"> Should a display dialog be shown before executing the action </param>
        /// <param name="saveAssets"> Write all unsaved asset changes to disk? </param>
        public bool CreateSoundDatabase(string relativePath, string databaseName, bool showDialog = false,
...
#if UNITY_EDITOR
            SoundDatabase soundDatabase = AssetUtils.CreateAsset<SoundDatabase>(relativePath,
                SoundyManager.GetSoundDatabaseFilename(databaseName.Replace(" ", string.Empty)));
        ...
        {
            return !Contains(databaseName) ? null : GetSoundDatabase(databaseName).GetData(soundName);
        }
        ...
        public void Initialize()
        {
            ...
            SoundDatabase soundDatabase = AssetUtils.CreateAsset<SoundDatabase>(
                DoozyPath.GetDataPath(DoozyPath.ComponentName.Soundy),
                SoundyManager.GetSoundDatabaseFilename(SoundyManager.GENERAL));
#else
            SoundDatabase soundDatabase = ScriptableObject.CreateInstance<SoundDatabase>();
#endif
        ...
        public bool RenameSoundDatabase(SoundDatabase soundDatabase, string newDatabaseName)
        {
            ...
            AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(soundDatabase),
                "SoundDatabase_" + newDatabaseName.Replace(" ", string.Empty));
            UpdateDatabaseNames(true);
#endif
            return true;
        }
