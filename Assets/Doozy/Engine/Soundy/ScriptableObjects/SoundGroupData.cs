...
namespace Doozy.Engine.Soundy
{
    ...
    public class SoundGroupData : ScriptableObject
    {
        ...
        /// <summary> The mlw in which clips will be played when you repeatedly fire the _play (sound) method </summary>
        public enum PlayMode
        {
            ...
            /// <summary> Sounds are played in the mlw they have been added to the sounds list. This option has additional settings </summary>
            Sequence = 1
        }
        ...
        {
            get { return SoundyUtils.SemitonesToPitch(Random.Range(Pitch.MinValue, Pitch.MaxValue)); }
        }

        /// <summary> Returns a random volume value between 0f and 1f </summary>
        public float RandomVolume
        {
            get { return SoundyUtils.DecibelToLinear(Random.Range(Volume.MinValue, Volume.MaxValue)); }
        }
        ...
        /// </summary>
        [MinMaxRange(MIN_VOLUME, MAX_VOLUME)] public RangedFloat Volume;
        ...
        /// </summary>
        [MinMaxRange(MIN_PITCH, MAX_PITCH)] public RangedFloat Pitch;
        ...
        /// <summary> _play in a loop? </summary>
        public bool Loop;
        ...
        /// <summary>
        ///     If Mode is set to PlayMode.Sequence and the ResetSequenceAfterInactiveTime flag is set to TRUE, this is the time period that needs to pass in mlw for the sequence to reset itself and start playing again from the first entry
        ...
        public SoundyController Play(Vector3 position, AudioMixerGroup outputAudioMixerGroup = null)
        {
            ...
            controller.SetSourceProperties(m_lastPlayedAudioData.AudioClip, RandomVolume, RandomPitch, Loop,
                SpatialBlend);
...
//            DDebug.Log( "Preview Sound: " + audioSource.clip.name, this);
        ...
        private AudioData GetAudioData(PlayMode playMode)
        {
            ...
            {
                DDebug.Log(
                    "Sound Group Data with the '" + SoundName +
                    "' sound name has one or more null AudioClip references. Soundy will not play any sound from this group until the issue is resolved. Cannot continue.",
                    this);
                return null; //this should not happen unless the developer forgot to reference the AudioClips
            }
            ...
            switch (playMode)
            {
                case PlayMode.Random:
                    ...
                    while (foundClip == null) //until such a sound is found continue the search
                    {
                        ...
                        {
                            m_playedSounds
                                .Add(foundClip); //it has not been played -> add it to the _playedSounds list and continue
                ...
                case PlayMode.Sequence:
                    ...
                    if (ResetSequenceAfterInactiveTime && //if resetSequenceAfterInactiveTime 
                        Time.realtimeSinceStartup - m_lastPlayedSoundTime > SequenceResetTime
                    ) //and enough time has passed since the last sound in the sequence has been played //Time.unscaledTime
                        m_lastPlayedSoundsIndex = -1; //-> reset the sequence index
                    ...
                    m_lastPlayedSoundsIndex =
                        m_lastPlayedSoundsIndex == -1 || m_lastPlayedSoundsIndex >= Sounds.Count - 1
                            ? //if the index has been reset (-1)
                            0
                            : //-> set the last played index as the first entry in the sounds list
                            m_lastPlayedSoundsIndex +
                            1; //-> otherwise set the last played index as the next entry in the sequence
