...
namespace Doozy.Engine.Soundy
{
    ...
    public class SoundyPooler : MonoBehaviour
    {
        ...
        {
            get { return SoundyManager.Pooler; }
        }
        ...
        {
            get { return s_pool ?? (s_pool = new List<SoundyController>()); }
            set { s_pool = value; }
        }
        ...
        {
            get { return SoundySettings.Instance.AutoKillIdleControllers; }
        }
        ...
        {
            get { return SoundySettings.Instance.MinimumNumberOfControllers; }
        }
        ...
        {
            get { return DoozySettings.Instance.DebugSoundyPooler; }
        }
        ...
        public static void ClearPool(bool keepMinimumNumberOfControllers = false)
        {
            if (keepMinimumNumberOfControllers)
            {
                ...
                ) //make sure the minimum number of controllers are in the pool before killing them
                {
                ...
                if (Instance.DebugComponent)
                    DDebug.Log(
                        "Clear Pool - Killed " + killedControllersCount + " Controllers - " + Pool.Count +
                        "' Controllers Available", Instance);
        ...
        /// <summary> GetParent a SoundyController from the Pool, or create a new one if all the available controllers are in use </summary>
        public static SoundyController GetControllerFromPool()
        {
            RemoveNullControllersFromThePool();
            if (Pool.Count <= 0)
                PutControllerInPool(SoundyController
                    .GetController()); //the pool does not have any controllers in it -> create and return a new controller
        ...
        public static void PopulatePool(int numberOfControllers)
        {
            ...
            if (Instance.DebugComponent)
                DDebug.Log(
                    "Populate Pool - Added " + numberOfControllers + " Controllers to the Pool - " + Pool.Count +
                    " Controllers Available", Instance);
        }
        ...
        public static void PutControllerInPool(SoundyController controller)
        {
            ...
            if (Instance.DebugComponent)
                DDebug.Log(
                    "Put '" + controller.name + "' Controller in the Pool - " + Pool.Count + " Controllers Available",
                    Instance);
        }
