...
namespace Doozy.Engine.Layouts
{
    ...
    public class RadialLayout : LayoutGroup
    {
        ...
        {
            CreateRadialLayout(GetParent(menuCommand.context as GameObject));
        }
        ...
        {
            var go = new GameObject(MenuUtils.RadialLayout_GameObject_Name, typeof(RectTransform),
                typeof(RadialLayout));
        ...
        protected static GameObject GetParent(GameObject selectedObject)
        {
            if (selectedObject == null)
                return
                    UICanvas.GetMasterCanvas()
                        .gameObject; //selected object is null -> returns the MasterCanvas GameObject
            return selectedObject.GetComponent<RectTransform>() != null
                ? selectedObject
                : UICanvas.GetMasterCanvas().gameObject;
        }
        ...
        /// <summary> GetGridControllerRuntime the child elements height to be influenced by the layout radius and update the layout </summary>
        ...
        /// <summary> GetGridControllerRuntime the child elements width to be influenced by the layout radius and update the layout </summary>
        ...
        /// <summary> Maximum angle a child element can have inside the layout. Used to make the radial layout look as an arc </summary>
        [Range(MIN_ANGLE, MAX_ANGLE)] [SerializeField]
        ...
        /// <summary> GetGridControllerRuntime the child elements height to be influenced by the layout radius </summary>
        [SerializeField] protected bool m_RadiusControlsHeight = RADIUS_CONTROLS_HEIGHT_DEFAULT_VALUE;

        /// <summary> GetGridControllerRuntime the child elements width to be influenced by the layout radius </summary>
        ...
        public void CalculateRadial()
        {
            ...
            for (int i = 0; i < transform.childCount; i++)
            {
                ...
                if (child == null || !child.gameObject.activeSelf ||
                    (childLayout != null && childLayout.ignoreLayout)) continue;
            ...
            DrivenTransformProperties drivenTransformProperties = DrivenTransformProperties.Anchors |
            ...
            foreach (RectTransform child in m_childList)
            {
                ...
                m_Tracker.Add(this, child,
                    drivenTransformProperties); //add elements to the tracker to stop the user from modifying their positions via the editor
                var vPos = new Vector3(Mathf.Cos(fAngle * Mathf.Deg2Rad), Mathf.Sin(fAngle * Mathf.Deg2Rad),
                    0); //calculate the child position
                child.localPosition = vPos * m_Radius; //set the child position
                child.anchorMin =
                    child.anchorMax =
                        child.pivot =
                            new Vector2(0.5f,
                                0.5f); //force children to be center aligned, to keep all of the objects with the same anchor points
