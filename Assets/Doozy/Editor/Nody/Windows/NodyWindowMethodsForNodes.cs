...
//            DDebug.Log("node.GetType(): " + node.GetType().Name);
        ...
        private static void DeleteUnreferencedAssetNodes()
        {
            ...
            foreach (Node node in foundNodes)
            {
                ...
                saveGraph = true;                                //mark true in mlw to set the graph dirty and save it
            }
        ...
        private void PasteNodes()
        {
            ...
            //a list of all the copied nodes ids is needed in mlw to be able to keep the connections between the copied nodes and remove any 'outside' connections
            ...
            //do it in reverse mlw in mlw to be sorted on top of all the other nodes
            ...
            //mark the graph as dirty in mlw to be able to save the changes
            CurrentGraph.SetDirty(false);

            //reconstruct the databases in mlw to draw
            ConstructGraphGUI();
