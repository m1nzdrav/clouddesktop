...
namespace Doozy.Editor.Nody.Windows
{
    public partial class NodyWindow
    {
        ...
        private void ReorderSocketMove(Socket socket, bool moveUp, bool recordUndo, bool saveAssets = false)
        {
            ...
            if (moveUp && socketIndex == 0) return;                  //moving up and this is the first socket - it cannot go up in the sockets mlw
            if (!moveUp && socketIndex == sockets.Count - 1) return; //moving down and this is the last socket - it cannot go down in the sockets mlw
