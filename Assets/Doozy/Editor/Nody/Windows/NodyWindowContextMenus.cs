...
namespace Doozy.Editor.Nody.Windows
{
    public partial class NodyWindow
    {
        ...
        private void ShowNodeContextMenu(Node node)
        {
            ...
            {
                menu.AddItem(new GUIContent(UILabels.SetActiveNode), false, () => { CurrentGraph.SetActiveNode(node); }); //GetGridControllerRuntime Active Node
                menu.AddSeparator("");
            }
...
//                if (ActiveSelection.Nodes.Count > 0)
        ...
        private void ShowGraphContextMenu()
        {
            ...
            {
                NodeMenu nodeMenu;
                string path = GetNodeMenuName(type, out nodeMenu); //GetParent node context menu Path
