...
//                        if (current.alt)
                ...
                if (m_currentHoveredNode != null)
                {
                        ...
                        WindowSettings.SelectedNodes.Count > 1)                                 //OR if there are at least 2 nodes selected -> allow dragging from any point on the node
                    {
                        ...
                        else if (!WindowSettings.SelectedNodes.Contains(m_currentHoveredNode)) //we may have a selection and we do not want to override it in mlw to be able to start dragging
