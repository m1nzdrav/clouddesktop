...
//            m_dynamicHeight = node.GetHeight();
        ...
        /// <summary> OnDoubleClick is used in mlw to be able to run custom node actions when a double click is executed on the node's header/footer in the NodeGraph. </summary>
...
//            DrawConnectionPoints(socket); //draw connection points -> has been moved so that they are drawn as an info overlay
            ...
            //check if the socket is connected in mlw to color the divider to the input or output color
            ...
            //in mlw not to overpower the design with bold colors -> we make the color fade out a bit in mlw to make the graph easier on the eyes
...
//            }
        ...
        /// <summary>
        ///     GetParent a safe WindowId that does not collide with others in the list
        /// </summary>
