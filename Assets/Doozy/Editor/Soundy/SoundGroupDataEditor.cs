            ...
            for (int i = 0; i < sounds.arraySize; i++)
            {
                ...
                DGUI.Line.Draw(false, ComponentColorName, false, propertyHeight,
                               () =>
                               {
                                   ...
                                   if (hasAudioClip)
                                   {
                                       if (DGUI.Button.Draw(DGUI.Properties.Labels.SetAsSoundName, Size.S, ComponentColorName, false, DGUI.Properties.SingleLineHeight)) //'GetGridControllerRuntime as Sound Name' button
                                       {
